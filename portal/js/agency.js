document.addEventListener('DOMContentLoaded', function () {
  ...
  const VIEWS = ['overview', 'customers', 'requests', 'services', 'tasks'];
  const HEADINGS = { overview: 'Overview', customers: 'Customers', requests: 'Service Requests', services: 'Services', tasks: 'Delivery Tasks' };
  ...
  function showView(view) {
    ...
    if (view === 'services') renderAgencyServices();
  ...
  function renderAgencyServices() {
    const tbody = document.querySelector('#serviceTable tbody');
    const rows = ZP.data.services.slice().sort((a, b) => a.name.localeCompare(b.name));
    if (!rows.length) {
      tbody.innerHTML = `<tr><td colspan="7" class="table-empty">No services yet — convert a request to create one.</td></tr>`;
      return;
    }
    tbody.innerHTML = rows.map(s => {
      const c = zpGetCustomer(s.customerId);
      const margin = s.billing.customerPrice - s.billing.ourCost;
      const cycle = s.billing.model === 'recurring' ? `/${s.billing.cycle === 'monthly' ? 'mo' : s.billing.cycle}` : ' one-time';
      return `<tr>
        <td>${zpCategoryIcon(s.category)} ${s.name}</td>
        ...
        <td>${zpFormatCurrency(s.billing.customerPrice)}${cycle}</td>
        <td style="color:var(--success);font-weight:600;">${zpFormatCurrency(margin)}${cycle}</td>
        <td><button class="btn btn-secondary btn-sm" data-edit-svc="${s.id}">Edit</button></td>
      </tr>`;
    }).join('');
    tbody.querySelectorAll('[data-edit-svc]').forEach(btn => {
      btn.addEventListener('click', () => openServiceEditor(btn.getAttribute('data-edit-svc')));
    });
  }
  ...
  function openServiceEditor(svcId) {
    ...
    document.getElementById('se_status').value = s.status;
    document.getElementById('se_model').value = s.billing.model;
    document.getElementById('se_provider').value = s.provider || '';
    document.getElementById('se_plan').value = s.plan || '';
    document.getElementById('se_cycle').value = s.billing.cycle || 'monthly';
    document.getElementById('se_renewal').value = s.billing.nextRenewal || '';
    document.getElementById('se_cost').value = s.billing.ourCost;
    document.getElementById('se_price').value = s.billing.customerPrice;
    document.getElementById('se_deployed').value = s.deployedDate || '';
    toggleSeCycle();
    document.getElementById('serviceModal').classList.add('open');
  }

  function toggleSeCycle() {
    document.getElementById('se_cycleField').style.display =
      document.getElementById('se_model').value === 'recurring' ? '' : 'none';
  }
  document.getElementById('se_model').addEventListener('change', toggleSeCycle);

  document.getElementById('serviceSaveBtn').addEventListener('click', () => {
    const s = zpGetService(editingSvcId);
    if (!s) return;
    const name = document.getElementById('se_name').value.trim();
    const ourCost = parseFloat(document.getElementById('se_cost').value);
    const customerPrice = parseFloat(document.getElementById('se_price').value);
    if (!name || isNaN(ourCost) || isNaN(customerPrice)) return;
    const model = document.getElementById('se_model').value;
    s.name = name;
    s.status = document.getElementById('se_status').value;
    s.provider = document.getElementById('se_provider').value.trim() || null;
    s.plan = document.getElementById('se_plan').value.trim() || null;
    s.deployedDate = document.getElementById('se_deployed').value || null;
    s.billing = model === 'recurring'
      ? { model: 'recurring', cycle: document.getElementById('se_cycle').value, ourCost, customerPrice, currency: s.billing.currency || 'USD', nextRenewal: document.getElementById('se_renewal').value || null }
      : { model: 'one-time', ourCost, customerPrice, currency: s.billing.currency || 'USD' };
    zpPersist();
    document.getElementById('serviceModal').classList.remove('open');
    renderAgencyServices();
    renderCustomers();
    renderOverview();
  });
  ...
  document.getElementById('serviceDeleteBtn').addEventListener('click', () => {
    const s = zpGetService(editingSvcId);
    if (!s) return;
    const taskCount = zpGetTasksForService(s.id).length;
    const invCount = zpGetInvoicesForService(s.id).length;
    if (!window.confirm(`Delete “${s.name}”? This also removes ${taskCount} linked task${taskCount === 1 ? '' : 's'}. ${invCount} invoice${invCount === 1 ? '' : 's'} are kept for the books.`)) return;
    ZP.data.services = ZP.data.services.filter(x => x.id !== s.id);
    ZP.data.tasks = ZP.data.tasks.filter(t => t.serviceId !== s.id);
    zpPersist();
    document.getElementById('serviceModal').classList.remove('open');
    renderAgencyServices();
    renderCustomers();
  ...
  document.getElementById('quoteSubmitBtn').addEventListener('click', () => {
    ...
    if (createInvoice) {
      const billingDefaults = zpSettings().billing;
      const depositRate = billingDefaults.depositPercent / 100;
      const amount = model === 'recurring' ? customerPrice : Math.round(customerPrice * depositRate * 100) / 100;
      const due = new Date(ZP_TODAY);
      due.setDate(due.getDate() + billingDefaults.paymentTermsDays);
