...
const ZP_SEED = {
  ...
  teamUsers: [
    { id: 'team-1', name: 'Imran Qureshi', email: 'imran@zainpreneur.agency', password: 'team1234', role: 'Infrastructure Engineer', skills: ['Linux', 'Docker', 'Odoo', 'Hostinger'], mfaEnabled: true },
  ...
  // Support tickets — customer-raised issues/requests against a deployed service.
  // Message threads stay in the open; resolution is a status change (open →
  // in-progress → resolved), mirroring the future backend tickets API.
  tickets: [
    {
      id: 'SUP-3182', customerId: 'cust-a', serviceId: 'svc-002',
      subject: 'Add employee access to Odoo', priority: 'normal', status: 'in-progress',
      createdDate: '2026-09-04', updatedDate: '2026-09-05',
      messages: [
        { author: 'Sarah Kim', date: '2026-09-04', text: 'Two new cashiers start Monday — they need Odoo POS access with the cashier role.' },
        { author: 'Zain Ahmed', date: '2026-09-05', text: 'On it — creating the accounts today and will confirm once they can log in.' }
      ]
    },
    {
      id: 'SUP-3174', customerId: 'cust-a', serviceId: 'svc-001',
      subject: 'Configure automated database backup', priority: 'high', status: 'resolved',
      createdDate: '2026-09-02', updatedDate: '2026-09-03',
      messages: [
        { author: 'Sarah Kim', date: '2026-09-02', text: 'Please set up nightly automated backups of the Odoo database with 14-day retention.' },
        { author: 'Imran Qureshi', date: '2026-09-03', text: 'Done — nightly backup verified and restore tested on staging.' }
      ]
    },
    {
      id: 'SUP-3179', customerId: 'cust-b', serviceId: 'svc-003',
      subject: 'SSL warning on checkout page', priority: 'high', status: 'open',
      createdDate: '2026-09-05', updatedDate: '2026-09-05',
      messages: [
        { author: 'Marcus Ortega', date: '2026-09-05', text: 'Customers report a browser SSL warning on the checkout page since this morning.' }
      ]
    }
  ],

  // Documents & evidence — metadata only (name, kind, link). File bytes are
  // outside frontend scope until object storage lands server-side.
  documents: [
    {
      id: 'doc-101', customerId: 'cust-a', serviceId: 'svc-001',
      title: 'Odoo go-live handover', kind: 'handover', fileName: 'odoo-handover.pdf',
      fileSize: '1.2 MB', notes: 'Admin credentials, backup schedule, support contacts.',
      createdDate: '2024-03-10', updatedDate: '2024-03-10'
    },
    {
      id: 'doc-102', customerId: 'cust-a', serviceId: 'svc-002',
      title: 'Inventory sync architecture', kind: 'report', fileName: 'sync-architecture.pdf',
      fileSize: '640 KB', notes: 'Sprint 1 design: Odoo XML-RPC ↔ Shopify webhooks.',
      createdDate: '2026-06-18', updatedDate: '2026-06-18'
    },
    {
      id: 'doc-103', customerId: 'cust-b', serviceId: 'svc-003',
      title: 'SSL install evidence', kind: 'evidence', fileName: 'ssl-check.png',
      fileSize: '210 KB', notes: 'Checkout page serving valid certificate.',
      createdDate: '2026-08-15', updatedDate: '2026-08-15'
    }
  ],
...
function zpLoadData() {
  ...
  if (!data) {
    data = zpClone(ZP_SEED);
    localStorage.setItem(ZP_STORAGE_KEY, JSON.stringify(data));
    return data;
  }
  // Forward-migrate data saved before newer seed collections existed
  // (e.g. tickets): backfill any missing top-level keys from the seed.
  let migrated = false;
  Object.keys(ZP_SEED).forEach(k => {
    if (data[k] === undefined) { data[k] = zpClone(ZP_SEED[k]); migrated = true; }
  });
  if (migrated) zpSaveData(data);
  return data;
}
...
/** Agency-wide settings with safe defaults; runtime-extended, never reseeded. */
function zpSettings() {
  const d = {
    general: { agencyName: 'Zainpreneur Agency', supportEmail: 'support@zainpreneur.agency' },
    billing: { currency: 'USD', paymentTermsDays: 7, depositPercent: 30 },
    notifications: { defaultLevel: 'all' },
    catalogue: { staleDays: 90 },
    security: { sessionTimeoutMins: 480, mfaRequired: false }
  };
  if (!ZP.data.settings) ZP.data.settings = {};
  const s = ZP.data.settings;
  Object.keys(d).forEach(k => { s[k] = Object.assign({}, d[k], s[k] || {}); });
  return s;
}
...
function zpGetTicket(id) { return ZP.data.tickets.find(t => t.id === id) || null; }
function zpGetTicketsForCustomer(customerId) {
  return ZP.data.tickets.filter(t => t.customerId === customerId);
}
function zpGetDocument(id) { return (ZP.data.documents || []).find(d => d.id === id) || null; }
function zpGetDocumentsForCustomer(customerId) {
  return (ZP.data.documents || []).filter(d => d.customerId === customerId);
}
function zpGetDocumentsForService(serviceId) {
  return (ZP.data.documents || []).filter(d => d.serviceId === serviceId);
}
...
const ZP_STATUS_LABEL = {
  ...
  open: 'Open', resolved: 'Resolved', done: 'Done',
  'needs-maintenance': 'Needs Maintenance', legacy: 'Legacy', 'at-risk': 'At Risk',
  unsupported: 'Unsupported', 'replacement-candidate': 'Replacement Candidate',
  'under-review': 'Under Review', retired: 'Retired'
};
