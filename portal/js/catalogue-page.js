document.addEventListener('DOMContentLoaded', function () {
  const session = zpInitShell({ role: ['team', 'agency'], active: 'catalogue', navItems: (s) => zpCatalogueNav(s.type) });
  if (!session) return;
  ...
  /** DB rows use a different shape (name/period) — normalize to the mock
      tool shape the renderers expect. Falls back to mock store offline. */
  function normalizeTool(t) {
    return {
      ...
      plans: (t.plans || []).map(p => ({
        id: slugify(p.name || 'plan'),
        label: p.name,
        price: parseFloat(p.price) || 0,
        currency: 'USD',
        cycle: p.period || 'monthly',
        limits: p.limits || {},
        features: p.features || [],
        limitations: []
      })),
      capabilities: t.capabilities || [],
      lastVerified: (t.lastRefreshedAt || '').slice(0, 10),
      dataSource: t.refreshSource || 'live API'
    };
  }

  async function getTools() {
    const mock = ZP.data.catalogueTools || {};
    if (Api.mode === 'rest') {
      try {
        const items = await Api.rest.catalog.list();
        // Hybrid: mock catalogue as the base so categories never lose tools
        // the backend doesn't track yet; live rows override by slug (then id).
        const map = { ...mock };
        items.forEach(t => {
          const n = normalizeTool(t);
          if (n.slug) map[n.slug] = n;
          map[n.id] = n;
        });
        return map;
  ...
  async function renderCategories() {
    const fetched = await getTools();
    currentTools = fetched;
    currentSource = Api.mode === 'rest' ? 'rest' : 'mock';
    const lookup = id => fetched[id] || null;
    document.getElementById('categoriesContainer').innerHTML = ZP_SERVICE_CATEGORIES.map(def => {
      let body;
      if (def.tools.length) {
        body = def.tools.map(lookup).filter(Boolean).map(renderToolBlock).join('');
  ...
  function openPricingModal(toolId) {
    ...
    const tool = currentTools[toolId] || zpGetTool(toolId);
    if (!tool) return;
  ...
  document.getElementById('pricingSaveBtn').addEventListener('click', async () => {
    ...
    if (currentSource === 'rest') {
      try {
        await Api.rest.catalog.update(editingToolId, {
          plans: plans.map(p => ({ name: p.label, price: String(p.price), period: p.cycle, features: p.features, limits: p.limits }))
        });
      } catch (e) {
        // Tool not tracked server-side (mock-only entry): keep the edit local.
